  ...
  try {
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "```json Analyze the uploaded image, along with any additional textual description, focusing on the predominant food item if mentioned. YOU CAN ONLY RETURN THE RESULT IN JSON STRING with the following content!!! //If the input value is not food, the JSON format file will not be returned. Instead, the analyzed item name will be returned  , displayed in json format 'none'='string' and display language Chinese.{'food': 'string', // Main food item, language setting is forever Traditional Chinese.'ingredient': 'string', // An analysis will be conducted based on the 'food' attribute to determine if this item is an original food item. If it is, the value of 'ingredient' will output the name of that original food. If it is not, an analysis will be performed to identify the top three food items that make up the largest proportions, and the value of 'ingredient' will output these top three predominant food items. If there are fewer than three types of food, all food items will be output.'ingredient_Ｅ' :'string',//copy 'ingredient' value and translate to English. 'result': { // Scores are based on the standard of 500 grams of white rice. All foods are calculated based on 500 grams. Scores range from 0 to 10. If there are units or percentages, the result is displayed according to the unit rules, instead of from 0 to 10.'methane': {'methane': int, // Scale of methane generated after food consumption, ranging from 0 to 10.'car_distance': int // Distance traveled by a car using methane fuel generated, unit: meters.},{'electricity': {'electricity_level': int, // Electricity level generated based on the amount of methane produced, ranging from 0 to 10.'phone_battery': int // Calculate the amount of electricity (in mAh) generated from the obtained methane. Then, divide this generated electricity by 3367 mAh (iPhone 15 battery capacity) and display the result as a percentage.}},'constipate': int, // Represents the potential constipation index based on food fiber content, presented on a scale of 0 to 10. Higher fiber values correspond to lower constipation indexes.'calorie': {'calorie_value': int, // Calorie content of the food, presented on a scale of 0 to 10.'run_distance': int // Distance required to run to burn the calories from the food, unit: meters.},{ 'suggest': 'string' // Provide suggestions on how to eat more environmentally friendly with a lower carbon footprint, using a humorous and accurate tone. 30 to 50 words, output language is always Traditional Chinese. }'data':'string' //Based on all the key values in this JSON file, only return the names of the websites used in the analysis, ensuring that the data comes from reputable academic journals or official scientific institutions.display language English. }}```"
        },
        {
          role: "user",
          content: [
